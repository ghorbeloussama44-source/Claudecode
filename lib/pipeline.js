...
const DEFAULTS = {
  ...
  // Defaults assume `ffmpeg`/`ffprobe` are on PATH (true for local dev with
  // system ffmpeg installed). The Vercel function passes the bundled
  // ffmpeg-static/ffprobe-static binary paths instead, since those aren't
  // present in a serverless runtime.
  ffmpegPath: 'ffmpeg',
  ffprobePath: 'ffprobe',
};
...
async function probeDuration(file, ffprobePath) {
...
async function detectSilences(file, { noise, minDuration, ffmpegPath }) {
  let stderr = '';
...
async function normalizeSegment(file, start, end, outPath, { resolution, fps, audioRate, ffmpegPath }) {
...
async function concatSegments(segmentPaths, listPath, outPath, ffmpegPath) {
  const listContent = segmentPaths.map((p) => `file '${p.replace(/'/g, "'\\''")}'`).join('\n');
  await fs.writeFile(listPath, listContent, 'utf8');
  await run(ffmpegPath, ['-y', '-f', 'concat', '-safe', '0', '-i', listPath, '-c', 'copy', outPath]);
}

async function applyLoudnorm(inPath, outPath, cfg, ffmpegPath) {
  ...
  await run(ffmpegPath, ['-y', '-i', inPath, '-map', '0:v', '-map', '0:a', '-af', af, '-c:v', 'copy', outPath]);
}
...
async function assembleFromRushes(inputPaths, outputPath, overrides = {}) {
  ...
  try {
    ...
    for (const file of inputPaths) {
      const duration = await probeDuration(file, opts.ffprobePath);
    ...
    await concatSegments(normalizedParts, listPath, concatenated, opts.ffmpegPath);
    await applyLoudnorm(concatenated, outputPath, opts.loudnorm, opts.ffmpegPath);

    stats.finalSeconds = await probeDuration(outputPath, opts.ffprobePath);
